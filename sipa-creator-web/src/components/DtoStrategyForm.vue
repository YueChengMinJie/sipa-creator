  ...
  <el-form ref="editForm" :rules="rules" :model="form" label-width="230px">
    ...
    <el-form-item prop="mapperPackage" label="Mapper(Xml)的包名">
      <el-autocomplete
          ...
          placeholder="例如：mapper.OrderMapper"
      >
        <el-select
      ...
      <help-tip
          content="生成的Mapper方法所在的Mapper.xml的包名，例如：mapper.OrderMapper"
      ></help-tip>
    </el-form-item>
    ...
    <el-form-item label="封装查询结果的DTO对象" prop="fullPackage">
      ...
      ></el-input>
      <help-tip
          content="用于映射查询的结果的DTO类，如果为空，则默认使用Map封装查询结果。需包含完整的包和类名称，不存在该类时会自动生成"></help-tip>
    </el-form-item>
...
export default {
  ...
  mounted: function () {
    axios.get("/api/sql/base-package").then((res) => {
