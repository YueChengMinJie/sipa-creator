...
export default {
  ...
  mounted: function () {
    this.getAllTables(this.tenant);
    this.getUserConfig();
    this.getIsNewProject();
  },
  methods: {
    ...
    genCode() {
      this.$confirm(
        ...
        data.genSetting = this.genSetting;
        data.tables = this.choosedTables;
        let aLoading = Loading.service();
        axios({
          method: 'post',
          url: '/api/mbp-generator/gen-code',
          data,
          responseType: "blob"
        })
            .then((blob) => {
